...
public class AtomAnalysis {
    ...
    public String atomToOMathAtomALL(List<Atom> atoms) {
        for (int i = 0; i < atoms.size(); i++) {
            ...
            if (atom.getMoveType() != null && atom.getMoveType().equals(1)) {
                continue;
            }
            ...
            if (atom.getAnalType().equals(1)) {
                if (atomToOMathSeriesAtom(atoms, i)) {
                    continue;
                }
    ...
    public String atomAnalysis(List<Atom> atoms, Integer i) {
        ...
        if (atomRule == null) {
            ...
            if (atomRule == null) {
                return "ERROR:解析标记:[" + atom.getAtomName() + "]没有对应的解析规则";
            }
                ...
                if (i + step > i && atomNext != null) {
                    ...
                    if (atomNext.getMoveType() == null) {
                        ...
                        if (atomNext.getAnalType() != null && atomNext.getAnalType().equals(1) && atomNext.getAtomName() != null) {
                            if (atomToOMathSeriesAtom(atoms, i + step)) {
                                continue;
                            }
                ...
                if (atomRuleBETmp.getMatchType().equals(0)) {
                    ...
                    if (atomRuleBETmp.getMappingType() != null && atomRuleBETmp.getMappingType().equals(3)) {//先处理匹配下一个块级元素，如 /left,/right
                        ...
                        if (groupBeginEnd != null && groupBeginEnd.equals(1) && (atom.getBlockType() == null || !atom.getBlockType().equals(1))) {//块级下沉
                            ...
                            if (group != null && groupBeginEnd != null && groupBeginEnd.equals(1))
                                groupTmpMap.put(group, groupTmpMap.get(group) == null ? 1 : groupTmpMap.get(group) + 1);
                            for (int blockStep = i + step + 1; blockStep < atoms.size(); blockStep++) {
                                ...
                                if (atomNextTmp != null && atomNextTmp.getAnalType() != null && atomNextTmp.getAnalType().equals(2))
                                    continue;
                                if (atomNextTmp != null && atomNextTmp.getAnalType().equals(1) && atomNextTmp.getAtomName() != null) {
                                    ...
                                    if (groupNext != null && groupNexBeginEnd != null && groupNexBeginEnd.equals(1))
                                        groupMap.put(groupNext, groupMap.get(groupNext) == null ? 1 : groupMap.get(groupNext) + 1);
                                    if (groupNext != null && groupNexBeginEnd != null && groupNexBeginEnd.equals(2))
                                        groupMap.put(groupNext, groupMap.get(groupNext) == null ? 0 : groupMap.get(groupNext) - 1);
                                    if (groupNext != null && groupNexBeginEnd != null && groupNexBeginEnd.equals(1))
                                        groupTmpMap.put(groupNext, groupTmpMap.get(groupNext) == null ? 1 : groupTmpMap.get(groupNext) + 1);
                                    if (groupNext != null && groupNexBeginEnd != null && groupNexBeginEnd.equals(2))
                                        groupTmpMap.put(groupNext, groupTmpMap.get(groupNext) == null ? 0 : groupTmpMap.get(groupNext) - 1);
                                ...
                                if (atomNextTmp.getAtomBEs() != null && atomNextTmp.getAtomBEs().size() > 0) {
                                    for (AtomBE atomBEChild : atomNextTmp.getAtomBEs()) {
                                        ...
                                        if (atomChild.getAtomName() != null && !atomChild.getAtomName().equals("") && atomChild.getAnalType().equals(1) && atomChild.getAtomRule() == null) {
                                            ...
                                            if (atomNextTmpRuleChild == null)
                                                return "ERROR:解析标记:[" + atomChild.getAtomName() + "]没有对应的解析规则";
                                            ...
                                            if (groupNextChild != null && groupNexBeginEndChild != null && groupNexBeginEndChild.equals(1))
                                                groupMap.put(groupNextChild, groupMap.get(groupNextChild) == null ? 1 : groupMap.get(groupNextChild) + 1);
                                            if (groupNextChild != null && groupNexBeginEndChild != null && groupNexBeginEndChild.equals(2))
                                                groupMap.put(groupNextChild, groupMap.get(groupNextChild) == null ? 0 : groupMap.get(groupNextChild) - 1);
                                            if (groupNextChild != null && groupNexBeginEndChild != null && groupNexBeginEndChild.equals(1))
                                                groupTmpMap.put(groupNextChild, groupTmpMap.get(groupNextChild) == null ? 1 : groupTmpMap.get(groupNextChild) + 1);
                                            if (groupNextChild != null && groupNexBeginEndChild != null && groupNexBeginEndChild.equals(2))
                                                groupTmpMap.put(groupNextChild, groupTmpMap.get(groupNextChild) == null ? 0 : groupTmpMap.get(groupNextChild) - 1);
                                        }
                                        if (atomChild.getAtomBEs() != null && atomChild.getAtomBEs().size() > 0) {
                                            for (AtomBE atomBEGrandSon : atomChild.getAtomBEs()) {
                                                ...
                                                if (atomGrandSon.getAtomName() != null && !atomGrandSon.getAtomName().equals("") && atomGrandSon.getAnalType().equals(1) && atomGrandSon.getAtomRule() == null) {
                                                    ...
                                                    if (atomNextTmpRuleGrandSon == null) {
                                                        return "ERROR:解析标记:[" + atomGrandSon.getAtomName() + "]没有对应的解析规则";
                                                    }
                                                    ...
                                                    if (groupNextGrandson != null && groupNexBeginEndGrandSon != null && groupNexBeginEndGrandSon.equals(1)) {
                                                        groupMap.put(groupNextGrandson, groupMap.get(groupNextGrandson) == null ? 1 : groupMap.get(groupNextGrandson) + 1);
                                                    }
                                                    if (groupNextGrandson != null && groupNexBeginEndGrandSon != null && groupNexBeginEndGrandSon.equals(2)) {
                                                        groupMap.put(groupNextGrandson, groupMap.get(groupNextGrandson) == null ? 0 : groupMap.get(groupNextGrandson) - 1);
                                                    }
                                                    if (groupNextGrandson != null && groupNexBeginEndGrandSon != null && groupNexBeginEndGrandSon.equals(1)) {
                                                        groupTmpMap.put(groupNextGrandson, groupTmpMap.get(groupNextGrandson) == null ? 1 : groupTmpMap.get(groupNextGrandson) + 1);
                                                    }
                                                    if (groupNextGrandson != null && groupNexBeginEndGrandSon != null && groupNexBeginEndGrandSon.equals(2)) {
                                                        groupTmpMap.put(groupNextGrandson, groupTmpMap.get(groupNextGrandson) == null ? 0 : groupTmpMap.get(groupNextGrandson) - 1);
                                                    }
                                                }
                                            }
                                        }
        ...
        if (atomRuleBEsTmp != null && atomRuleBEsTmp.size() > 0 && step == -99) {//解析类似\over将整个块从中间分隔的样式
            ...
            List<AtomBE> atomBeforeNewBEs = new ArrayList<>();
        ...
        for (AtomBE atomRuleSeriesAtomBE : atomRuleSeriesList) {
            ...
            if (atomRuleSeriesAtomBE.getMatchType() != null && atomRuleSeriesAtomBE.getMatchType().equals(0)) {//可以不匹配时处理
                ...
                if (atomRuleSeriesAtomBE.getAtom() == null || atomRuleSeriesAtomBE.getAtom().getAtomName() == null) {
                    ...
                    if (currentAtom.getAtomName() == null || !currentAtom.getAtomName().equals(atomRuleSeriesAtomBE.getAtom().getAtomName())) {
                        return false;
                    }
                    step = step == -1 ? 1 : step + 1;
                }
    ...
    public String atomToOMathAtom(Atom atom) {
        ...
        if (atom.getAtomBEs() != null && atom.getAtomBEs().size() > 0) {
            ...
            if (atomChildren != null && atomChildren.size() > 0) {
                message = atomToOMathAtomALL(atomChildren);
            }
            if (!message.equals("SUCCESS")) {
                return message;
            }
